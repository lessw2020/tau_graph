...
def print_graph(name, graph):
    if DEBUG:
        ...
        print(f"===== end graph print for {name} ====")
...
def make_functional_fx(fn):
    ...
    def call(*args, **kwargs):
        ...
        with torch.no_grad():
            ...
            print(f"141 passed")
            res = functionalize(k2)  # , decomposition_table=all_decomps
            print(f"143 passed, {res=}")
            k3 = make_fx(res, decomposition_table=all_decomps)
            print(f"143 passed, type k3 = {type(k3)}")
            print(f"{k3=}")
