...
class Scheduler:
    ...
    def __init__(self, nodes):
        ...
        for i, node in enumerate(nodes):
            # print(f"619, type {type(node)}")
            # if rank == 0:
            # print(f"index {i}, node name = {node=}")

            assert (
                node.origins is not None
            ), "All nodes passed to scheduling must have an origin"

            # screen for allreduce and wait
            if isinstance(node, (ir.AllReduce)):

                # print(f"type size {type(size)}, {size=}, {total=}")
                # print(
                #     f"\n====> found AllReduce - {node.get_name()}, {node.get_layout()}"
                # )

                self.allreduces.append((i, node.get_name()))

            elif isinstance(node, ir.Wait):
                # print(f"\n===> found Wait, {node.get_name()}, {node.get_layout()}")
                layout = node.get_layout()
                dtype = layout.dtype
                size = layout.size
                # print(f"type size {type(size)}, {size=}")
                self.waits.append((i, node.get_name()))
            ...
            if node.is_no_op():
                ...
                # special case allred and wait
                extern_node = ExternKernelSchedulerNode(self, node)
                if isinstance(node, ir.AllReduce):
                    self.allred_map[node.get_name()] = extern_node
                elif isinstance(node, ir.Wait):
                    self.waits_map[node.get_name()] = extern_node

                self.nodes.append(extern_node)
            else:
        ...
        # move second wait back
        if self._rank == 0:
            for index, name in self.waits:
                # name = node.get_name()
                print(f"index {index}, name = {name}")
            print(f"----- end wait list ------")
        self.move_last_wait()
    ...
    def track_collectives(self):
        return
        self.waits = []
        self.allreduces = []
        for i, node in enumerate(self.nodes):
            if isinstance(node, ir.AllReduce):
                self.allreduces.append((i, node))
            elif isinstance(node, ir.Wait):
                self.waits.append((i, node))

    def move_last_wait(self):
        index1, node1 = self.waits[0]
        index2, node2 = self.waits[-1]

        shifted_nodes = []
        for i, node in enumerate(self.nodes):
            name = node.get_name()
            if name not in self.waits_map:
                shifted_nodes.append(node)
                continue
            if name == node1:
                print(f"not adding {name}")
                continue  # don't add
            if name == node2:
                # add in first wait
                shifted_nodes.append(self.waits_map[node1])
                shifted_nodes.append(node)

        if self._rank == 0:
            print(
                f"747, len of nodes {len(self.nodes)}, vs shifted {len(shifted_nodes)}"
            )

        assert len(shifted_nodes) == len(self.nodes), "mismatch between node lists"
        if self._rank == 0:
            for i, (old_node, new_node) in enumerate(zip(self.nodes, shifted_nodes)):

                print(f"index {i} before: {old_node}, now {new_node}")
    ...
    def view_graph(self):
        ...
        for i, node in enumerate(self.nodes):
            name = node.get_name()
            # if name in self.collective_map:

            print(f"index {i}, {name}, type = {type(node)}")

        print(f" ---- end graph -------\n")

    def debug_draw_graph(self):
        """Generate an image of the graph for debugging"""
        if os.environ.get("INDUCTOR_WRITE_SCHEDULER_GRAPH", 1) == "1":
