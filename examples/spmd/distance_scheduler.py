...
os.environ["INDUCTOR_WRITE_SCHEDULER_GRAPH"] = "0"
...
class BaseSchedulerNode:
    def __init__(self, scheduler: "Scheduler", node: ir.Buffer):
        ...
        self.last_usage: Set[
            str
        ] = None  # buffers that won't be used after this kernel
        self.written = False
        ...
        for use in users:
            if id(use.node) in result:
                result[id(use.node)] = NodeUser(
                    use.node,
                    result[id(use.node)].can_inplace and use.can_inplace,
                )
            else:
...
class SchedulerNode(BaseSchedulerNode):
    def __init__(
        self, scheduler: "Scheduler", node: ir.ComputedBuffer, group_fn
    ):
    ...
    def pointwise_read_writes(self):
        ...
        def fn(index):
            return self._body(
                index, [sympy.Integer(0) for _ in reduction_sizes]
            )
    ...
    def can_inplace(self, read_dep: dependencies.MemoryDep):
        ...
        if len(self.read_writes.writes) == 1 and hasattr(read_dep, "index"):
            ...
            return (
                read_dep.index == write_dep.index
...
class FusedSchedulerNode(BaseSchedulerNode):
    ...
    def debug_str_extra(self):
        return f"{self.get_name()}.snodes = {pformat([x.get_name() for x in self.snodes])}"

    @cache_on_self
    def used_buffer_names(self) -> Set[str]:
        return functools.reduce(
            set.union, [x.used_buffer_names() for x in self.snodes]
        )
...
class Scheduler:
    ...
    def __init__(self, nodes):
        ...
        if _is_fwd:
            print(f"entering forward pass scheduling")
            _is_fwd = False
        else:
            print(f"entering bwd pass scheduling")
            _is_fwd = True
        print(
            f"l_569, class Scheduler inited, with len nodes {incoming_node_len}\n"
        )
    ...
    def compute_dependencies(self):
        ...
        def add_user(used_by_name, user_node, can_inplace=False):
            name_to_users[rename(used_by_name)].append(
                NodeUser(user_node, can_inplace)
            )
    ...
    def fuse_nodes(self):
        ...
        for i in range(10):
            ...
            fused_count = old_len - new_len
            if fused_count:
                print(
                    f"l_798, fusion pass {i}, fused {fused_count} nodes, start len {old_len}, new_len = {new_len}"
                )
    ...
    def will_fusion_create_cycle(self, node1, node2):
        ...
        return any(
            check(self.name_to_fused_node[n]) for n in combined_predecessors
        )
    ...
    def can_fuse(self, node1: BaseSchedulerNode, node2: BaseSchedulerNode):
        ...
        if (
            isinstance(
                node1, (ExternKernelSchedulerNode, NopKernelSchedulerNode)
            )
        ...
        if (
            isinstance(
                node2, (ExternKernelSchedulerNode, NopKernelSchedulerNode)
            )
    ...
    def can_fuse_vertical(self, node1, node2):
        ...
        for name in remaining_deps:
            if (
                node1_names
                & self.name_to_fused_node[name].recursive_predecessors
            ):
                return False
        return True
    ...
    def compute_last_usage(self):
        ...
        for node in reversed(self.nodes):
            ...
            used_buffers = {
                self.mutation_real_name.get(k, k) for k in used_buffers
            }
