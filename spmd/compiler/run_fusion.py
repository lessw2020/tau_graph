...
def work_main(rank, world_size):
    ...
    #mesh = DeviceMesh(_device_type, mesh_tensor)

    #local_tensor = torch.ones(2, 2, device=_device_type) * self.rank

    #message(rank, mesh, title="device mesh")

    #reduced_tensor = device_mesh.all_reduce(local_tensor, mesh_dim=0)
    #message(rank, reduced_tensor, "reduced_tensor")

    #schema = Schema()

    #spmd = SPMD(sample,  schema)

    #message(rank, spmd, title="spmd")

    def fn_all_reduce(mesh, tensor: torch.Tensor):
        reduced_tensor = mesh.all_reduce(tensor, mesh_dim=1)
    ...
    #traced_fn = make_fx(fn)(mesh, local_tensor + 1)

    #red_tensor = fn_all_reduce(mesh, local_tensor)
    ...
    for dim, dim_group in enumerate(dim_to_subgroups):
        ...
        # use a local_tensor + 1 for tracing to make sure that we are not
        # simply replaying recorded tensor value
        traced_fn = make_fx(fn_ag)(all_gather_base_tensor + 1, (dim_group_size * 3, 3))
        gathered_tensor = traced_fn(all_gather_base_tensor, (dim_group_size * 3, 3))
        ...
        print(f"{rank} --> test all gather.  exp_tensor = {exp_tensor}\n gathered tensor = {gathered_tensor}")
